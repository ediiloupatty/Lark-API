...
const getCustomers = async (req, res) => {
    ...
    `;
        ...
        const customers = await db_1.db.$queryRawUnsafe(query, ...params);
...
const addCustomer = async (req, res) => {
    try {
        ...
        if (!nama)
            return res.status(400).json({ status: 'error', message: 'Nama pelanggan wajib diisi.' });
        // Cek duplikasi no hp
        if (no_hp) {
            const existing = await db_1.db.$queryRawUnsafe(`SELECT id FROM customers WHERE tenant_id = $1 AND no_hp = $2 AND deleted_at IS NULL`, tenantId, no_hp);
            if (existing.length > 0) {
                return res.status(400).json({ status: 'error', message: 'Nomor HP sudah terdaftar.' });
            }
        }
        const inserted = await db_1.db.$queryRawUnsafe(`INSERT INTO customers (tenant_id, nama, no_hp, alamat, email, client_id, server_version)
       VALUES ($1, $2, $3, $4, $5, $6, CAST(EXTRACT(EPOCH FROM NOW()) * 1000 AS BIGINT))
       RETURNING id, client_id, nama, no_hp, alamat, email, server_version`, tenantId, nama, no_hp || null, alamat || null, email || null, client_id || null);
...
const updateCustomer = async (req, res) => {
    try {
        ...
        if (!id && !client_id)
            return res.status(400).json({ status: 'error', message: 'ID pelanggan diperlukan.' });
        // Pengecekan constraint unik untuk no_hp jika diubah
        if (no_hp) {
            const existing = await db_1.db.$queryRawUnsafe(`SELECT id FROM customers WHERE tenant_id = $1 AND no_hp = $2 AND deleted_at IS NULL AND id != $3 AND (client_id != $4 OR client_id IS NULL)`, tenantId, no_hp, id || -1, client_id || 'UNKNOWN');
            if (existing.length > 0) {
                return res.status(400).json({ status: 'error', message: 'Nomor HP sudah digunakan pelanggan lain.' });
            }
        }
        const updated = await db_1.db.$queryRawUnsafe(`UPDATE customers SET 
       ...
       RETURNING id, client_id, nama, no_hp, alamat, email, server_version`, nama, no_hp || null, alamat || null, email || null, tenantId, id || -1, client_id || 'UNKNOWN');
        if (updated.length === 0) {
...
const deleteCustomer = async (req, res) => {
    try {
        ...
        // Soft delete
        const deleted = await db_1.db.$queryRawUnsafe(`UPDATE customers SET deleted_at = NOW(), server_version = CAST(EXTRACT(EPOCH FROM NOW()) * 1000 AS BIGINT)
       WHERE tenant_id = $1 AND (id = $2 OR client_id = $3)
       RETURNING id`, tenantId, id || -1, client_id || 'UNKNOWN');
        if (deleted.length === 0) {
            return res.status(404).json({ status: 'error', message: 'Pelanggan tidak ditemukan.' });
        }
        res.json({
