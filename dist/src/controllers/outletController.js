...
exports.deleteOutlet = exports.updateOutlet = exports.addOutlet = exports.getOutlets = void 0;
...
const getOutlets = async (req, res) => {
    try {
        ...
        const outletsRaw = await db_1.db.$queryRawUnsafe(query, ...params);
...
const addOutlet = async (req, res) => {
    try {
        ...
        if (!nama)
            return res.status(400).json({ status: 'error', message: 'Nama outlet wajib diisi.' });
        const inserted = await db_1.db.$queryRawUnsafe(`INSERT INTO outlets (tenant_id, nama, alamat, phone, jam_buka, jam_tutup)
       VALUES ($1, $2, $3, $4, $5::time, $6::time) RETURNING id`, tenantId, nama, alamat || null, phone || null, jam_buka || '08:00', jam_tutup || '20:00');
        res.status(201).json({
            status: 'success',
...
const updateOutlet = async (req, res) => {
    try {
        ...
        const updated = await db_1.db.$queryRawUnsafe(`UPDATE outlets SET nama = $1, alamat = $2, phone = $3, jam_buka = $4::time, jam_tutup = $5::time
       WHERE id = $6 AND tenant_id = $7 RETURNING id`, nama, alamat || null, phone || null, jam_buka || '08:00', jam_tutup || '20:00', parseInt(id), tenantId);
        if (updated.length === 0)
...
const deleteOutlet = async (req, res) => {
    try {
        ...
        const { id } = req.body;
        if (!id)
            return res.status(400).json({ status: 'error', message: 'ID Outlet diperlukan.' });
        await db_1.db.$queryRawUnsafe(`DELETE FROM outlets WHERE id = $1 AND tenant_id = $2`, parseInt(id), tenantId);
