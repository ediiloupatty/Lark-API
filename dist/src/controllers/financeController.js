...
const updateExpense = async (req, res) => {
    try {
        ...
        if (!kategori || !jumlah)
            return res.status(400).json({ status: 'error', message: 'Kategori dan jumlah wajib diisi.' });
        // Verify ownership
        const existing = await db_1.db.expenses.findFirst({ where: { id: expId, tenant_id: tenantId } });
        if (!existing)
...
const getReports = async (req, res) => {
    try {
        ...
        const startDate = req.query.start_date || firstOfMonth;
        const endDate = req.query.end_date || todayStr;
        // Outlet filter
        let outletId = req.user?.outlet_id || null;
        const reqOid = req.query.oid ? parseInt(req.query.oid) : null;
        if (reqOid !== null) {
        ...
        prevStart.setDate(prevStart.getDate() - Math.max(dayCount - 1, 0));
        const prevStartStr = prevStart.toISOString().split('T')[0];
        const prevEndStr = prevEnd.toISOString().split('T')[0];
        // Scope label
        let scopeLabel = 'Semua Outlet';
        if (outletId) {
            const ot = await db_1.db.$queryRawUnsafe(`SELECT nama FROM outlets WHERE tenant_id = $1 AND id = $2`, tenantId, outletId);
            if (ot.length > 0)
                scopeLabel = ot[0].nama;
        }
        // --- Summary current period ---
        const summaryRows = await db_1.db.$queryRawUnsafe(`
      SELECT
    ...
    `, tenantId, startDate, endDate);
        ...
        const prevRows = await db_1.db.$queryRawUnsafe(`
      SELECT
        COUNT(CASE WHEN o.status != 'dibatalkan' THEN 1 END) AS billable_orders,
        COALESCE(SUM(CASE WHEN o.status != 'dibatalkan' THEN o.total_harga ELSE 0 END), 0) AS gross_revenue
      FROM orders o
    ...
    `, tenantId, prevStartStr, prevEndStr);
        ...
        // --- Expenses ---
        const expRows = await db_1.db.$queryRawUnsafe(`
    ...
    `, tenantId, startDate, endDate);
        ...
        // --- Status breakdown ---
        const statusRaw = await db_1.db.$queryRawUnsafe(`
    ...
    `, tenantId, startDate, endDate);
        ...
        // --- Payment breakdown ---
        const payRows = await db_1.db.$queryRawUnsafe(`
    ...
    `, tenantId, startDate, endDate);
        ...
        for (const r of payRows)
            payIndex[r.payment_status] = r;
        const PAYMENT_LABELS = { lunas: 'Lunas', pending: 'Belum Lunas', dibatalkan: 'Pembayaran Dibatalkan' };
        ...
        // --- Payment methods ---
        const methodRows = await db_1.db.$queryRawUnsafe(`
    ...
    `, tenantId, startDate, endDate);
        ...
        // --- Chart data ---
        const chartRaw = await db_1.db.$queryRawUnsafe(`
    ...
    `, tenantId, startDate, endDate);
        ...
        // --- Top services ---
        const topServices = await db_1.db.$queryRawUnsafe(`
    ...
    `, tenantId, startDate, endDate);
        // --- Top customers ---
        const topCustomers = await db_1.db.$queryRawUnsafe(`
    ...
    `, tenantId, startDate, endDate);
        ...
        let topStaff = await db_1.db.$queryRawUnsafe(`
    ...
    `, tenantId, startDate, endDate);
        ...
        if (topStaff.length === 0) {
            topStaff = await db_1.db.$queryRawUnsafe(`
        SELECT u.nama,
      ...
      `, tenantId);
        ...
        let topOutlets = [];
        if (!outletId && isAdmin) {
            topOutlets = await db_1.db.$queryRawUnsafe(`
...
const getPayments = async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        const payments = await db_1.db.$queryRaw `
            SELECT p.id, 
