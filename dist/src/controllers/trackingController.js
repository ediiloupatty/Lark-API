...
class TrackingController {
        ...
        try {
            ...
            return res.status(200).json({
                ...
                data: {
                    ...
                    payment_status: order.payments.length > 0 ? order.payments[0].status_pembayaran : 'pending'
                }
            });
        }
        catch (error) {
    ...
    static async verifyTracking(req, res) {
        try {
            ...
            const history = await db_1.db.orders.findMany({
                where: {
            ...
            return res.status(200).json({
                ...
                data: {
                    ...
                    history: history.map(h => ({
                        ...
                        items: h.order_details.map((item) => ({
                            layanan: item.services?.nama_layanan || item.jenis_pakaian,
                            jumlah: item.jumlah,
                            berat: item.berat
                        })),
