...
const ipaymuService_1 = require("../services/ipaymuService");
class PaymentController {
    // Endpoint to create a test transaction for iPaymu Reviewers
    static async createTestTransaction(req, res) {
        try {
            const { productName, price } = req.body;
            const appUrl = process.env.APP_URL || 'http://localhost:5173';
            const paymentData = await ipaymuService_1.IpaymuService.createPayment({
