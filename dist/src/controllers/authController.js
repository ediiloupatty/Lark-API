...
exports.googleLogin = exports.resetPassword = exports.forgotPassword = exports.registerAdmin = exports.loginStaff = exports.loginAdmin = exports.logoutAdmin = void 0;
...
const loginAdmin = async (req, res) => {
    ...
    try {
        ...
        if (user && user.deleted_at === null && user.password && (await (0, authUtils_1.verifyModernPassword)(trimmedPassword, user.password))) {
            const role = (0, authUtils_1.normalizeAppRole)(user.role);
            if (role === 'karyawan') {
                (0, rateLimiter_1.recordFailedLogin)(ipAddress);
            ...
            // Rehash if necessary
            if ((0, authUtils_1.needsPasswordRehashUpgrade)(user.password)) {
                const newHash = await (0, authUtils_1.hashPassword)(trimmedPassword);
                await db_1.db.users.update({
                    where: { id: user.id },
            ...
            // Success
            (0, rateLimiter_1.clearFailedLogin)(ipAddress);
            await (0, auditHelper_1.writeAuditLog)(db_1.db, {
...
const loginStaff = async (req, res) => {
    ...
    try {
        ...
        const user = await db_1.db.users.findFirst({
            where: {
        ...
        if (user) {
            ...
            const token = jsonwebtoken_1.default.sign({
                user_id: user.id,
                username: user.username,
                role: (0, authUtils_1.normalizeAppRole)(user.role),
                tenant_id: user.tenant_id,
                outlet_id: user.outlet_id,
                token_version: user.token_version ?? 0,
            }, jwtSecret, { expiresIn: '24h' });
...
const registerAdmin = async (req, res) => {
    try {
        ...
        const existingUser = await db_1.db.users.findUnique({
            where: { username: username.trim() }
        });
        if (existingUser) {
            return res.status(400).json({ success: false, error: 'Username sudah digunakan, coba username lain!' });
        }
        const hashedPw = await (0, authUtils_1.hashPassword)(password.trim());
...
const forgotPassword = async (req, res) => {
    try {
        ...
        try {
            await (0, mailer_1.sendPasswordResetEmail)(cleanEmail, resetUrl, user.nama || user.username);
        }
        catch (mailErr) {
...
const resetPassword = async (req, res) => {
    try {
        ...
        const newHash = await (0, authUtils_1.hashPassword)(password.trim());
        await db_1.db.users.update({
            where: { id: user.id },
            data: {
                password: newHash,
...
const googleLogin = async (req, res) => {
    ...
    try {
        ...
        let user = await db_1.db.users.findFirst({
        ...
        if (!user) {
            user = await db_1.db.users.findFirst({
        ...
        if (!user) {
            ...
            while (await db_1.db.users.findUnique({ where: { username: finalUsername } })) {
            ...
            let slug = baseSlug;
            let slugSuffix = 1;
            while (await db_1.db.tenants.findUnique({ where: { slug: slug } })) {
