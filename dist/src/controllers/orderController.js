...
exports.deleteOrder = exports.payOrder = exports.updateOrderStatus = exports.createOrder = exports.getOrders = void 0;
...
const getOrders = async (req, res) => {
    try {
        ...
        const limit = Math.min(Math.max(1, parseInt(req.query.limit || '20')), 100); // Fix BH-2: max 100
    ...
    `;
        ...
        if (orderIdStr) {
            query += ` AND o.id = $2`;
            const orders = await db_1.db.$queryRawUnsafe(query, tenantId, parseInt(orderIdStr));
            if (orders.length === 0) {
            ...
            let items = [];
            try {
                items = await db_1.db.$queryRawUnsafe(`SELECT od.*, s.nama_layanan as nama_item, s.harga_per_kg, pk.nama_paket as paket_nama
           FROM order_details od
           JOIN services s ON od.service_id = s.id
           LEFT JOIN paket_laundry pk ON s.paket_id = pk.id
           WHERE od.order_id = $1`, order.id);
            }
            catch (e) {
                items = await db_1.db.$queryRawUnsafe(`SELECT od.*, s.nama_layanan as nama_item, s.harga_per_kg, NULL as paket_nama
           FROM order_details od
           JOIN services s ON od.service_id = s.id
           WHERE od.order_id = $1`, order.id);
            }
        ...
        const orders = await db_1.db.$queryRawUnsafe(query, ...params);
        // Fetch order details mapping
        let itemsMap = {};
        if (orders.length > 0) {
            const orderIds = orders.map((o) => o.id);
            try {
                const items = await db_1.db.$queryRawUnsafe(`SELECT od.*, s.nama_layanan as nama_item, s.harga_per_kg, pk.nama_paket as paket_nama
           FROM order_details od
           JOIN services s ON od.service_id = s.id
           LEFT JOIN paket_laundry pk ON s.paket_id = pk.id
           WHERE od.order_id = ANY($1::int[])`, orderIds);
                ...
                const items = await db_1.db.$queryRawUnsafe(`SELECT od.*, s.nama_layanan as nama_item, s.harga_per_kg, NULL as paket_nama
           FROM order_details od
           JOIN services s ON od.service_id = s.id
           WHERE od.order_id = ANY($1::int[])`, orderIds);
                for (const item of items) {
...
const createOrder = async (req, res) => {
    try {
        ...
        const { customer_id, items, client_id, pickup_method, delivery_method, pickup_zone, delivery_zone, pickup_address, delivery_address, metode_bayar, status_bayar, metode_antar, catatan } = req.body;
        ...
        const verifyCust = await db_1.db.customers.findFirst({
        ...
        if (isAdmin && req.body.outlet_id) {
            const verifyOutlet = await db_1.db.outlets.findFirst({
        ...
        // Idempotency check
        if (client_id) {
            const existing = await db_1.db.$queryRawUnsafe(`SELECT id, tracking_code, total_harga, status FROM orders WHERE tenant_id = $1 AND client_id = $2`, tenantId, client_id);
        ...
        const result = await db_1.db.$transaction(async (tx) => {
            ...
            if (req.body.paket_id)
                orderData.paket_id = parseInt(req.body.paket_id);
            if (pickup_address)
                orderData.alamat_jemput = pickup_address;
            if (userId)
            ...
            await tx.payments.create({
                data: {
                    tenant_id: tenantId,
                    order_id: orderId,
                    metode_pembayaran: paymentMethodStr,
        ...
        (0, firebaseService_1.sendPushToAdmins)({
            tenantId: tenantId,
            title: pushTitle,
            body: pushBody,
            data: {
        ...
        db_1.db.users.findMany({
            where: { tenant_id: tenantId, role: { in: ['admin', 'super_admin', 'owner'] }, is_active: true, deleted_at: null },
            select: { id: true },
        }).then((admins) => {
            for (const admin of admins) {
                (0, firebaseService_1.saveNotification)({
                    tenantId: tenantId,
                    userId: admin.id,
        ...
        db_1.db.customers.findFirst({ where: { id: parseInt(customer_id), tenant_id: tenantId } })
            .then(async (cust) => {
            if (!cust?.no_hp)
                return;
            const tenant = await db_1.db.tenants.findUnique({ where: { id: tenantId }, select: { name: true } });
            const firstItem = Array.isArray(items) && items.length > 0 ? items[0] : null;
            const svc = firstItem?.service_id
                ? await db_1.db.services.findFirst({ where: { id: parseInt(firstItem.service_id) }, select: { nama_layanan: true } })
                : null;
            const msg = (0, whatsappService_1.buildNewOrderMessage)({
            ...
            (0, whatsappService_1.sendWhatsApp)({ tenantId: tenantId, phone: cust.no_hp, message: msg }).catch(() => { });
        })
            .catch(() => { });
    }
    catch (err) {
...
const updateOrderStatus = async (req, res) => {
    try {
        ...
        if (status === 'diproses')
            queryUpdates += `, tgl_diproses = NOW()`;
        if (status === 'siap_diambil' || status === 'siap_diantar')
            queryUpdates += `, tgl_siap = NOW()`;
        if (status === 'selesai')
            queryUpdates += `, tgl_selesai = NOW()`;
        queryUpdates += `, server_version = CAST(EXTRACT(EPOCH FROM NOW()) * 1000 AS BIGINT)`;
        const updated = await db_1.db.$queryRawUnsafe(`UPDATE orders SET ${queryUpdates} WHERE id = $2 AND tenant_id = $3 RETURNING id, status`, ...params);
        if (updated.length === 0)
            return res.status(404).json({ status: 'error', message: 'Pesanan tidak ditemukan.' });
        res.json({
            status: 'success',
        ...
        if (WA_NOTIFY_STATUSES.includes(status)) {
            db_1.db.$queryRawUnsafe(`SELECT o.tracking_code, c.no_hp, c.nama as nama_pelanggan, t.nama as nama_toko
         FROM orders o
         JOIN customers c ON o.customer_id = c.id
         JOIN tenants t ON o.tenant_id = t.id
         WHERE o.id = $1`, parseInt(String(id))).then(async (rows) => {
                if (!rows[0]?.no_hp)
                    return;
                const r = rows[0];
                const msg = (0, whatsappService_1.buildStatusUpdateMessage)({
                ...
                if (msg)
                    (0, whatsappService_1.sendWhatsApp)({ tenantId: tenantId, phone: r.no_hp, message: msg }).catch(() => { });
            }).catch(() => { });
        }
    }
    catch (err) {
...
const payOrder = async (req, res) => {
    try {
        ...
        const metodeBayar = VALID_PAYMENT_METHODS.includes(rawMethod) ? rawMethod : 'cash';
        if (!order_id) {
...
const deleteOrder = async (req, res) => {
    try {
        ...
        if (!orderId)
            return res.status(400).json({ status: 'error', message: 'ID pesanan diperlukan.' });
        // Verify order belongs to tenant
        const orderRows = await db_1.db.$queryRawUnsafe(`SELECT id, status FROM orders WHERE id = $1 AND tenant_id = $2`, orderId, tenantId);
        ...
        if (isAdmin) {
            ...
            await db_1.db.$queryRawUnsafe(`UPDATE orders SET status = 'dibatalkan', 
         server_version = CAST(EXTRACT(EPOCH FROM NOW()) * 1000 AS BIGINT),
         updated_at = NOW()
         WHERE id = $1 AND tenant_id = $2`, orderId, tenantId);
            // Batalkan pembayaran yang terkait
            await db_1.db.$queryRawUnsafe(`UPDATE payments SET status_pembayaran = 'dibatalkan' WHERE order_id = $1 AND tenant_id = $2`, orderId, tenantId);
        ...
        await db_1.db.$queryRawUnsafe(`UPDATE orders SET status = 'dibatalkan', server_version = CAST(EXTRACT(EPOCH FROM NOW()) * 1000 AS BIGINT), updated_at = NOW() WHERE id = $1 AND tenant_id = $2`, orderId, tenantId);
