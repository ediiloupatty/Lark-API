...
const pullChanges = async (req, res) => {
    try {
        ...
        // ── Services ────────────────────────────────────────────────
        if (await (0, SyncService_1.syncTableHasColumn)(db_1.db, 'services', 'server_version')) {
            let q = `
      ...
      `;
            const params = [tenantId, sinceVersion];
            if (!isAdmin && outletId !== null) {
                q += ` AND (outlet_id = $3 OR outlet_id IS NULL)`;
                params.push(outletId);
            }
            q += ` ORDER BY server_version ASC`;
            const rawServices = await db_1.db.$queryRawUnsafe(q, ...params);
            data.services = rawServices.map((s) => {
        ...
        if (await (0, SyncService_1.syncTableExists)(db_1.db, 'paket_laundry')) {
            let q = `
      ...
      `;
            ...
            const rawPkgs = await db_1.db.$queryRawUnsafe(q, ...params);
            data.packages = rawPkgs.map((p) => ({
                id: Number(p.id),
                nama: p.nama,
        ...
        // ── Customers ────────────────────────────────────────────────
        if (await (0, SyncService_1.syncTableHasColumn)(db_1.db, 'customers', 'server_version')) {
            const q = `
      ...
      `;
            data.customers = await db_1.db.$queryRawUnsafe(q, tenantId, sinceVersion);
        }
        // ── Orders ───────────────────────────────────────────────────
        if (await (0, SyncService_1.syncTableHasColumn)(db_1.db, 'orders', 'server_version')) {
            ...
            const orderRows = await db_1.db.$queryRawUnsafe(q, ...params);
            for (const row of orderRows) {
                const payload = await (0, SyncService_1.syncFetchOrderPayload)(db_1.db, tenantId, row.id);
                if (payload)
        ...
        try {
            const outletHasSV = await (0, SyncService_1.syncTableHasColumn)(db_1.db, 'outlets', 'server_version');
            if (outletHasSV && sinceVersion > 0) {
                // Incremental: hanya outlet yang berubah sejak sinceVersion
                const rows = await db_1.db.$queryRawUnsafe(`SELECT id, nama, alamat, phone, jam_buka, jam_tutup, is_active, server_version
           FROM outlets WHERE tenant_id = $1 AND server_version > $2
           ORDER BY server_version ASC`, tenantId, sinceVersion);
                data.outlets = rows.map((o) => ({ ...o, server_version: Number(o.server_version || 0), jam_buka: formatTime(o.jam_buka), jam_tutup: formatTime(o.jam_tutup) }));
            }
            else {
                // Bootstrap atau tabel tanpa server_version: kirim semua outlet aktif
                const rows = await db_1.db.$queryRawUnsafe(`SELECT id, nama, alamat, phone, jam_buka, jam_tutup, is_active
           FROM outlets WHERE tenant_id = $1 AND is_active = true
           ORDER BY nama ASC`, tenantId);
                data.outlets = rows.map((o) => ({ ...o, jam_buka: formatTime(o.jam_buka), jam_tutup: formatTime(o.jam_tutup) }));
            }
        }
        catch (outletErr) {
        ...
        if (data.customers.length >= 200 || data.orders.length >= 100) {
            ...
            data.server_version = await (0, SyncService_1.syncCurrentServerVersion)(db_1.db, tenantId);
        }
