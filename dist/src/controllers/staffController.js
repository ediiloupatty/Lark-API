...
exports.updateStaffPermissions = exports.updateGlobalPermissions = exports.getGlobalPermissions = exports.toggleStaffStatus = exports.deleteStaff = exports.updateStaff = exports.addStaff = exports.getStaff = void 0;
...
const getStaff = async (req, res) => {
    ...
    `;
        ...
        const users = await db_1.db.$queryRawUnsafe(query, ...params);
...
const addStaff = async (req, res) => {
    try {
        ...
        const exist = await db_1.db.$queryRawUnsafe(
        ...
        if (exist.length > 0)
            return res.status(400).json({ status: 'error', message: `Gagal! ID Akses '${finalUsername}' sudah digunakan.` });
        ...
        const hashedPassword = await bcrypt_1.default.hash(passwordPlain, 10);
        ...
        const inserted = await db_1.db.$queryRawUnsafe(`INSERT INTO users (tenant_id, username, password, nama, no_hp, alamat, role, outlet_id, is_active)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, true) RETURNING id`, tenantId, finalUsername, hashedPassword, nama, no_hp || '', alamat || '', 'karyawan', finalOutletId);
...
const updateStaff = async (req, res) => {
    try {
        ...
        const exist = await db_1.db.$queryRawUnsafe(`SELECT id FROM users WHERE username = $1 AND id != $2 AND tenant_id = $3`, finalUsername, finalUserId, tenantId);
        if (exist.length > 0)
            return res.status(400).json({ status: 'error', message: `Gagal! ID Akses '${finalUsername}' sudah digunakan akun lain.` });
        let finalOutletId = outlet_id ? parseInt(outlet_id) : null;
        if (finalOutletId) {
            // [SECURITY FIX] Cross-Tenant Outlet IDOR
            const verifyOutlet = await db_1.db.outlets.findFirst({ where: { id: finalOutletId, tenant_id: tenantId } });
        ...
        const params = [nama, finalUsername, no_hp || '', alamat || '', finalOutletId];
        let idx = 6;
        if (password && password.trim() !== '') {
            const hashedPassword = await bcrypt_1.default.hash(password.trim(), 10);
            query += `, password = $${idx++}`;
            params.push(hashedPassword);
        }
        query += ` WHERE id = $${idx++} AND tenant_id = $${idx}`;
        params.push(finalUserId, tenantId);
        const updated = await db_1.db.$executeRawUnsafe(query, ...params);
...
const deleteStaff = async (req, res) => {
    try {
        ...
        const finalUserId = parseInt(idParam);
        if (!finalUserId)
        ...
        await db_1.db.$queryRawUnsafe(`UPDATE users SET deleted_at = CURRENT_TIMESTAMP, is_active = false WHERE id = $1 AND tenant_id = $2`, finalUserId, tenantId);
...
const toggleStaffStatus = async (req, res) => {
    try {
        ...
        if (!id)
            return res.status(400).json({ status: 'error', message: 'ID diperlukan.' });
        await db_1.db.$queryRawUnsafe(`UPDATE users SET is_active = $1 WHERE id = $2 AND tenant_id = $3`, is_active ? true : false, parseInt(id), tenantId);
...
const getGlobalPermissions = async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        if (!tenantId)
            return res.status(403).json({ status: 'error', message: 'Tenant required.' });
        const rows = await db_1.db.$queryRawUnsafe(`SELECT setting_value FROM tenant_settings WHERE tenant_id = $1 AND setting_key = 'global_staff_permissions'`, tenantId);
...
const updateGlobalPermissions = async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        if (!tenantId)
            return res.status(403).json({ status: 'error', message: 'Tenant required.' });
        ...
        if (!perms)
            return res.status(400).json({ status: 'error', message: 'Permissions wajib diisi.' });
        ...
        const exist = await db_1.db.$queryRawUnsafe(`SELECT id FROM tenant_settings WHERE tenant_id = $1 AND setting_key = 'global_staff_permissions'`, tenantId);
        if (exist.length > 0) {
            await db_1.db.$queryRawUnsafe(`UPDATE tenant_settings SET setting_value = $1, updated_at = NOW() WHERE id = $2`, JSON.stringify(perms), exist[0].id);
        }
        else {
            await db_1.db.$queryRawUnsafe(`INSERT INTO tenant_settings (tenant_id, setting_key, setting_value, updated_at) VALUES ($1, 'global_staff_permissions', $2, NOW())`, tenantId, JSON.stringify(perms));
        }
        // Attempt to update all users
        try {
            await db_1.db.$queryRawUnsafe(`UPDATE users SET permissions = $1 WHERE tenant_id = $2 AND role = 'karyawan'`, JSON.stringify(perms), tenantId);
        }
        catch (e) { }
...
exports.updateGlobalPermissions = updateGlobalPermissions;
...
const updateStaffPermissions = async (req, res) => {
    try {
        ...
        if (!staff_id || !permissions)
            return res.status(400).json({ status: 'error', message: 'ID Staf dan permissions wajib diisi.' });
        const defaultPerms = { manage_orders: false, confirm_payments: false, view_reports: false, manage_expenses: false };
        const perms = { ...defaultPerms, ...permissions };
        await db_1.db.$queryRawUnsafe(`UPDATE users SET permissions = $1 WHERE id = $2 AND tenant_id = $3`, JSON.stringify(perms), parseInt(staff_id), tenantId);
